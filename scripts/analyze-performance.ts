...
function generateRecommendations(bundles: BundleStats[]): string[] {
  ...
  if (largeBundles.length > 0) {
    recommendations.push(
      `⚠️ Large bundles detected: ${largeBundles.map((b) => `${b.name} (${formatBytes(b.size)})`).join(", ")}`,
    )
  ...
  try {
    ...
    if (performanceCode.includes("DEFAULT_BUDGETS")) {
      ...
      if (appInitMatch) {
        ...
        console.log(
          `  - App Init Budget: ${budget}ms ${budget <= PERFORMANCE_THRESHOLDS.APP_INIT_BUDGET ? "✅" : "⚠️"}`,
        )
      }
      ...
      if (sidecarInitMatch) {
        ...
        console.log(
          `  - Sidecar Init Budget: ${budget}ms ${budget <= PERFORMANCE_THRESHOLDS.SIDECAR_INIT_BUDGET ? "✅" : "⚠️"}`,
        )
      }
    } else {
